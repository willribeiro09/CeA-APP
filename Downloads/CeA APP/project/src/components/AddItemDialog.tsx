...
export function AddItemDialog({ isOpen, onOpenChange, category, onSubmit, selectedWeekStart }: AddItemDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (category === 'Expenses') {
      ...
      // Normalizar a data de início para evitar problemas de fuso horário
      const startDateString = data.startDate as string;
      console.log("Data de início selecionada:", startDateString);
      
      // Criar um objeto Date normalizado a partir da string da data
      const startDate = startDateString ? normalizeDate(new Date(startDateString)) : normalizeDate(new Date());
      
      console.log("Data de início normalizada:", startDate.toISOString());
      
      // Normalizar a data da semana selecionada
      const weekStartDate = selectedWeekStart ? normalizeDate(selectedWeekStart) : normalizeDate(new Date());
