-- Solução alternativa para o problema de sincronização com o Supabase
...
-- 4. Criar um gatilho para manter lastSync e lastsync sincronizados
CREATE OR REPLACE FUNCTION sync_lastsync_trigger()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    IF NEW.lastsync IS NOT NULL AND NEW."lastSync" IS NULL THEN
      NEW."lastSync" := NEW.lastsync;
    ELSIF NEW."lastSync" IS NOT NULL AND NEW.lastsync IS NULL THEN
      NEW.lastsync := NEW."lastSync";
    END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS sync_lastsync ON sync_data;
CREATE TRIGGER sync_lastsync
BEFORE INSERT OR UPDATE ON sync_data
FOR EACH ROW
EXECUTE FUNCTION sync_lastsync_trigger();

-- 5. Inserir um registro inicial com valores válidos
...
-- 7. Reiniciar a publicação realtime
DROP PUBLICATION IF EXISTS supabase_realtime;
CREATE PUBLICATION supabase_realtime FOR TABLE sync_data;

-- 8. Verificar o conteúdo da tabela após as alterações
SELECT * FROM sync_data; 